...
parser = argparse.ArgumentParser(description='Hexdump data memory')
...
for i in range(0, 256, 2):
    datamem = datamem + list(dev.ctrl_transfer(USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_IN,
              CH341_REQ_READ_REG, 0x100 * (i + 1) + i, 0x0, 2))

hexdump.hexdump(bytes(datamem))
